...
#ifndef LSST_MEAS_BASE_SkyCoord_h_INCLUDED
#define LSST_MEAS_BASE_SkyCoord_h_INCLUDED

/**
 *  @file lsst/meas/base/SkyCoord.h
 *
 *  This file is one of two (the other is SdssShape.h) intended to serve as an tutorial example on
 *  how to implement new Algorithms.  SkyCoordAlgorithm is a particularly simple algorithm, while
...
/**
 *  @brief A C++ control class to handle SkyCoordAlgorithm's configuration
 *
...
/**
 *  @brief Additional results for SkyCoordAlgorithm
 *
 *  Unlike PsfFlux, some of SkyCoord's outputs aren't handled by the standard FluxComponent,
 ...
 *
 *  Note: for what I guess are historical reasons, SkyCoord computes covariance terms between the flux
 *  and the shape, but not between the flux and centroid or centroid and shape.
 *
 *  This should logically be an inner class, but Swig doesn't know how to parse those.
 */

class SkyCoordExtras {
public:
    SkyCoordExtras();
};
 ...
 *  This should logically be an inner class, but Swig doesn't know how to parse those.
 */
class SkyCoordExtrasMapper {
public:

    /**
     *  @brief Allocate fields in the schema and save keys for future use.
     *
     *  Unlike the standard ComponentMappers, SkyCoordExtrasMappers takes a Control instance
     ...
     */
    SkyCoordExtrasMapper(
        afw::table::Schema & schema,
        std::string const & prefix,
        SkyCoordControl const & control=SkyCoordControl()
    );

    /// Transfer values from the result struct to the record.
    void apply(afw::table::BaseRecord & record, SkyCoordExtras const & result) const;

private:
    afw::table::Key<lsst::afw::coord::Coord> _coord;
};

/**
 *  @brief A measurement algorithm that estimates flux using a linear least-squares fit with the Psf model
 *
 *  The SkyCoord algorithm is extremely simple: we do a least-squares fit of the Psf model (evaluated
 ...
 *  (see SkyCoordControl::usePixelWeights), as this results in bright stars being fit with a different
 *  effective profile than faint stairs.
 *
 *  As one of the simplest Algorithms, SkyCoord is documented to serve as an example in implementing new
 *  algorithms.  For an overview of the interface Algorithms should adhere to, see
 *  @ref measBaseAlgorithmConcept.
 *
 *  As an Algorithm class, all of SkyCoordAlgorithm's core functionality is available via static methods
 *  (in fact, there should be no reason to ever construct an instance).
 *
 *  Almost all of the implementation of SkyCoordAlgorithm is here and in SkyCoordAlgorithm.cc, but there
 *  are also a few key lines in the Swig .i file:
 *  @code
 *  %include "lsst/meas/base/SkyCoord.h"
 *  %template(apply) lsst::meas::base::SkyCoordAlgorithm::apply<float>;
 *  %template(apply) lsst::meas::base::SkyCoordAlgorithm::apply<double>;
 *  %wrapMeasurementAlgorithm1(lsst::meas::base, SkyCoordAlgorithm, SkyCoordControl, FootprintCentroidInput,
 *                             FluxComponent)
 *  @endcode
 *  and in the pure Python layer:
 *  @code
 *  WrappedSingleFramePlugin.generate(SkyCoordAlgorithm)
 *  @endcode
...
public:
    ...
    static boost::array<FlagDef,N_FLAGS> const & getFlagDefinitions() {
        static boost::array<FlagDef,N_FLAGS> const flagDefs = {{
                {"noWcs", "No Wcs object attached to the Exposure being measured"},
            }};
        return flagDefs;
    }
    ...
    typedef SkyCoordControl Control;

    /**
     *  This is the type returned by apply().  Because SkyCoordAlgorithm measure multiple fluxes,
     ...
     *  Exposure being measured.  SkyCoordAlgorithm only needs a centroid, so we use FootprintCentroidInput.
     */
    ...
    /**
     *  @brief Measure the flux of a source using the SkyCoord algorithm.
     *
    ...
    /**
     *  @brief Apply the SkyCoord to a single source using the Plugin API.
     *
...
#endif // !LSST_MEAS_BASE_SkyCoord_h_INCLUDED
