...
class ShapeTestCase(unittest.TestCase):
    ...
    def do_testmeasureShape(self):
        ...
        algorithm = measBase.SdssShapeAlgorithm
        ...
        for a, b, phi, tol in [              # n.b. phi in degrees
            ...
            algorithm.apply(exp.getMaskedImage(), footprint, footprint.getCentroid(), result, control)

            if False:
                Ixx, Iyy, Ixy = result.xx, result.yy, result.xy
            ...
            self.assertTrue(abs(x - result.x) < 1e-4, "%g v. %g" % (x, result.x))
            self.assertTrue(abs(y - result.y) < 1e-4, "%g v. %g" % (y, result.y))
            self.assertTrue(abs(result.xx - sigma_xx) < tol*(1 + sigma_xx),
                            "%g v. %g" % (sigma_xx, result.xx))
            self.assertTrue(abs(result.xy - sigma_xy) < tol*(1 + abs(sigma_xy)),
                            "%g v. %g" % (sigma_xy, result.xy))
            self.assertTrue(abs(result.yy - sigma_yy) < tol*(1 + sigma_yy),
                            "%g v. %g" % (sigma_yy, result.yy))
