...
class CentroidTestCase(lsst.utils.tests.TestCase):
    ...
    def do_testAstrometry(self, alg, bkgd, control=None):
        """Test that we can instantiate and play with a centroiding algorithms"""

        if control == None: control = alg.Control()
        x0, y0 = 12345, 54321
        for imageFactory in (afwImage.MaskedImageF,
            ...
            result = alg.Result()
            alg.apply(exp, afwGeom.Point2D(x+x0, y+y0),result,control)
            self.assertClose(x + x0, result.x, rtol=.000001)
            self.assertClose(y + y0, result.y, rtol=.000001)
...
class SingleFrameMeasurementTaskTestCase(lsst.utils.tests.TestCase):
    ...
    def mySetup(self, runCentroider=True):
        ...
        flags = measBase.MeasurementDataFlags()
        task = measBase.SingleFrameMeasurementTask(schema, flags, config=msConfig)
...
class MonetTestCase(unittest.TestCase):
	...
	im = afwImage.ImageF(self.monetFile("small.fits"))
        ...
        schema = afwTable.SourceTable.makeMinimalSchema()
        flags = measBase.MeasurementDataFlags()
        self.task = measBase.SingleFrameMeasurementTask(schema, flags, config=msConfig)
