...
class SFMTestCase(lsst.utils.tests.TestCase):
    ...
    def testAlgorithm(self):
        ...
        #  Basic test of NaiveFlux algorithm, no C++ slots
        sfm_config.plugins = ["base_PeakCentroid", "base_NaiveFlux"]
        sfm_config.slots.centroid = "base_PeakCentroid"
        sfm_config.slots.shape = None
        sfm_config.slots.psfFlux = None
        sfm_config.slots.instFlux = None
        sfm_config.slots.apFlux = None
        sfm_config.slots.modelFlux = "base_NaiveFlux"
        ...
        for i in range(len(measCat)):
            ...
            srcRec = srccat[i]
            # check all the flags
            self.assertFalse(record.get("base_NaiveFlux_flag"))
            # check the slots
