...
#include "lsst/meas/base/detail/SincPhotometry.h"
#include "lsst/meas/base/detail/Photometry.h"
...
SincFluxAlgorithm::ResultMapper SincFluxAlgorithm::makeResultMapper(
    ...
    if (detail::fuzzyCompare<float>().isEqual(ctrl.ellipticity, 0.0)) {
        detail::SincCoeffs<float>::cache(ctrl.radius1, ctrl.radius2);
    }
...
void SincFluxAlgorithm::apply(
    ...
    std::pair<double, double> fluxes =
        detail::calculateSincApertureFlux(exposure.getMaskedImage(),
...
#define INSTANTIATE(T)                                                  \
    ...
    detail::calculateSincApertureFlux<lsst::afw::image::MaskedImage<T> >(                            \
        lsst::afw::image::MaskedImage<T> const&, lsst::afw::geom::ellipses::Ellipse const&, double const);           \
    template class detail::SincCoeffs<T>;
