...
ApFluxComponentMapper::ApFluxComponentMapper(
    ...
    _flux = schema.addField(
        afw::table::Field<float>(prefix + "_flux", "measured flux", "dn"),
        true
    );
    if (uncertainty != NO_UNCERTAINTY) {
        _fluxSigma = schema.addField(
            afw::table::Field<float>(
...
void ApFluxComponentMapper::apply(afw::table::BaseRecord & record, ApFluxComponent const & result) const {
...
ApFluxComponent::ApFluxComponent() :
    ...
    )
{
    ...
    for (unsigned int i = 0; i < control.radii.size(); i++)
    {
        ...
        sstm << prefix << '.' << i;
        ApFluxComponentMapper fluxComponentMapper(schema, sstm.str(), SIGMA_ONLY); 
...
ApertureFluxAlgorithm::Result ApertureFluxAlgorithm::apply(
    ...
    for (unsigned int i = 0; i <  ctrl.radii.size(); ++i) {
        ...
        boost::shared_ptr<lsst::meas::base::ApFluxComponent> ptr = boost::shared_ptr<lsst::meas::base::ApFluxComponent>(new ApFluxComponent());
